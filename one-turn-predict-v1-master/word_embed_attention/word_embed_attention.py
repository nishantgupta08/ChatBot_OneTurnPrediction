...
class Seq2SeqModel(object):
    ...
    def build_encoder(self):
        ...
        with tf.variable_scope('encoder'):
            ...
            self.linear_layer_c = Dense(self.hidden_units, use_bias=True, name='decoder_cell_state_c_proj')
            self.linear_layer_h = Dense(self.hidden_units, use_bias=True, name='decoder_cell_state_h_proj')
        ...
        with tf.variable_scope('decoder'):
            ...
            if self.mode == 'train':
                ...
                training_decoder = seq2seq.BasicDecoder(cell=self.decoder_cell,
                                                   ...
                                                   initial_state=self.decoder_cell.zero_state(batch_size=self.batch_size, dtype=self.dtype).clone(cell_state=self.decoder_initial_state),
    ...
    def get_average_word_embed(self, cell_type='LSTM'):


        max_seq_len_batch = tf.reduce_max(self.encoder_inputs_length)

        input_masks = tf.sequence_mask(self.encoder_inputs_length, max_seq_len_batch)
        input_masks_expanded = tf.tile(tf.expand_dims(input_masks, 2), tf.stack([1, 1, self.hidden_units]))
        input_masks_expanded_numeric = tf.cast(input_masks_expanded, dtype=tf.float32)


        sent_mean_matrix = tf.reduce_sum(tf.multiply(self.encoder_inputs_embedded2, input_masks_expanded_numeric), axis=[1], keep_dims=True) / tf.reduce_sum(input_masks_expanded_numeric, axis=[1], keep_dims=True)
        sent_mean_matrix = tf.reshape(sent_mean_matrix, [self.batch_size, self.hidden_units])

        c_out = self.linear_layer_c(sent_mean_matrix)
        h_out = self.linear_layer_h(sent_mean_matrix)


        if self.mode == 'train':
            h_true_fn = tf.layers.dropout(h_out, rate=self.keep_prob_placeholder, training=True)
            h_false_fn = tf.layers.dropout(h_out, rate=self.keep_prob_placeholder, training=False)
            c_true_fn = tf.layers.dropout(c_out, rate=self.keep_prob_placeholder, training=True)
            c_false_fn = tf.layers.dropout(c_out, rate=self.keep_prob_placeholder, training=False)

            h_out_dropout = tf.cond(self.keep_prob_placeholder < 1.0, lambda: h_true_fn, lambda: h_false_fn)
            c_out_dropout = tf.cond(self.keep_prob_placeholder < 1.0, lambda: c_true_fn, lambda: c_false_fn)

        elif self.mode == 'decode':
            h_out_dropout = tf.layers.dropout(h_out, rate=self.keep_prob_placeholder, training=False)
            c_out_dropout = tf.layers.dropout(c_out, rate=self.keep_prob_placeholder, training=False)

        encoder_last_state = LSTMStateTuple(c_out_dropout, h_out_dropout)
    ...
    def build_decoder_cell(self):
        ...
        encoder_outputs = self.encoder_inputs_embedded2
        encoder_last_state = self.get_average_word_embed()
        ...
        decoder_cell = attention_wrapper.AttentionWrapper(
            cell=self.build_single_cell(),
