...
from vanilla_seq2seq import Seq2SeqModel
from tqdm import tqdm


# Context pre-processing
tf.app.flags.DEFINE_string('user_vocab_file', 'vocab_data/user_first_response_jan_2018_p1_16_12_2018.p', 'Path to encoder vocabulary')
tf.app.flags.DEFINE_string('agent_vocab_file', 'vocab_data/agent_first_response_jan_2018_p1_16_12_2018.p', 'Path to decoder vocabulary')

# Dataset
tf.app.flags.DEFINE_string('train_data', 'data/first-response-jan-2018-processed-p1.txt', 'Path to training data')
tf.app.flags.DEFINE_string('valid_data', 'data/first-response-jan-2018-processed-p1.txt', 'Path to validating data')
...
tf.app.flags.DEFINE_string('model_dir', 'model_08_06_2018/', 'Path to save model checkpoints')
...
def train():
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=FLAGS.allow_soft_placement,
        ...
        for epoch_idx in tqdm(range(FLAGS.max_epochs)):
            ...
            #Iterating over Iterator(This will call next)
            for batch_train_data in train_set:
                ...
                if model.global_step.eval() % FLAGS.display_freq == 0:
                    ...
                    print ('Epoch ', model.global_epoch_step.eval(), 'Step ', model.global_step.eval(), 'Perplexity {0:.2f}'.format(avg_perplexity), 'time-taken: ', round(time_elapsed,0), ' seconds')
                ...
                if valid_set and model.global_step.eval() % FLAGS.valid_freq == 0 and epoch_idx>9 and epoch_idx % 5==0:
                    ...
                    total_valid_time = time.time() - valid_start_time

                    valid_loss = valid_loss / (batch_indx+1)
                    print ('Valid perplexity: {0:.2f}'.format(math.exp(valid_loss)), ' Total validation time: ', round(total_valid_time,0), ' seconds')
